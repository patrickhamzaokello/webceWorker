...
$total_newCases_sql = mysqli_query($con, "SELECT COUNT(*) as newCases from cases where status = 1");
$row = mysqli_fetch_array($total_newCases_sql);
$total_newCases = $row['newCases'];

//get number of $approvedCases_sql
$approvedCases_sql = mysqli_query($con, "SELECT COUNT(*) as approvedCases from cases where status = 2");
$row = mysqli_fetch_array($approvedCases_sql);
$total_approvedCases = $row['approvedCases'];

//get number of $handledCases_sql
$handledCases_sql = mysqli_query($con, "SELECT COUNT(*) as handledCases from cases where status = 3");
...
$total_new_appointment_sql = mysqli_query($con, "SELECT COUNT(*) as total_new_appointment FROM  appointments WHERE status = 1");
...
$total_approved_appointment_sql = mysqli_query($con, "SELECT COUNT(*) as total_approved_appointment FROM appointments WHERE status = 2");
...
$total_canceled_appointment_sql = mysqli_query($con, "SELECT COUNT(*) as total_cancel_appointment FROM appointments WHERE status = 3");
